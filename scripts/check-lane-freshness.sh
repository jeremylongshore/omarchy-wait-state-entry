...
trap '/usr/bin/rm -r -- "$TMP_DIR"' EXIT
...
while read -r want file; do
  ...
  remote_file="$TMP_DIR/$(/usr/bin/printf '%s' "$file" | /usr/bin/tr '/' '_')"
  /usr/bin/curl -fsSL --max-time 20 -o "$remote_file" "$BASE/$file" 2>/dev/null
  rc=$?
  if [[ "$rc" -ne 0 ]]; then
  ...
  checked=$((checked + 1))
  # Hash the fetched file, not command-substitution output. POSIX command
  # substitution strips trailing newlines, so hashing `$body` made every
  # byte-identical shell script differ from canonical forever.
  got=$(/usr/bin/sha256sum "$remote_file" | /usr/bin/cut -d' ' -f1)
