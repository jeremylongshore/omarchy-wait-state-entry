...
RECEIPT="$TARGET/.rig-render-proof.json"
...
# Prove the plugin does not accidentally depend on language runtimes that are
# present on the rig but absent from a stock graphical session. Any attempted
# node/python/ruby invocation resolves to a loud failing shadow executable.
rm -rf /tmp/omarchy-no-interpreters; mkdir -p /tmp/omarchy-no-interpreters
for runtime in node nodejs python python3 ruby; do
  printf '#!/bin/sh\necho forbidden interpreter: %s >&2\nexit 126\n' "\$runtime" \
    > "/tmp/omarchy-no-interpreters/\$runtime"
  chmod 755 "/tmp/omarchy-no-interpreters/\$runtime"
done
PATH=/tmp/omarchy-no-interpreters:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin \
...
echo "===RUNTIME=== pid=\$QS_PID path=/tmp/omarchy-no-interpreters-first"
...
SHOT_SHA="$(sha256sum "$OUT" | cut -d' ' -f1)"
WARNING_COUNT="$(printf '%s\n' "$WARNINGS" | awk 'NF { n++ } END { print n+0 }')"
jq -n --arg fp "$FP" --arg rig "$HOST/$CONTAINER" --arg screenshot "$OUT" \
  --arg screenshotSha256 "$SHOT_SHA" --argjson screenshotBytes "$SIZE" \
  --argjson warnings "$WARNING_COUNT" --arg runtimePath "/tmp/omarchy-no-interpreters-first" \
  --argjson at "$(date +%s)" --arg iso "$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
  '{fingerprint:$fp, rig:$rig, realShellLoaded:true, qmlWarningCount:$warnings,
    interpreterShadowPath:$runtimePath, screenshot:$screenshot,
    screenshotSha256:$screenshotSha256, screenshotBytes:$screenshotBytes,
    renderedAtEpoch:$at, renderedAt:$iso}' > "$RECEIPT"
